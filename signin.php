...
if(isset($_POST['SignIn'])){   //clicked on signin or not
	...
	if(isset($_POST['email']) && !empty($_POST['email'])){
	 ...
	 if(isset($email) && isset($password)){
		...
		if(mysqli_num_rows($result)>0){
			...
			<strong>Sorry,No record found. Please fill the valid email or password.</strong>
			<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
			<span aria-hidden="true">&times;</span>
			</button>
		   </div>';
...
<div class="container size">
  ...
  <div class="form-group">
  <button type="submit" class="btn btn-danger btn-lg center-aligned" name="SignIn">SignIn</button>
  </div>
</form>
        </div>
    </div>
